...
- 项目工作流：支持新建、打开、保存、自动保存和最近 `.zclip` 项目恢复
- 多轨时间线：视频轨 + 音频轨，支持增减轨道、吸附、缩放、合并选择和历史记录
- 常用编辑能力：分割、复制/剪切/粘贴、删除、撤销/重做
- 画面编辑：支持画布尺寸/背景设置、片段位置/缩放/旋转，以及淡入淡出和交叉淡化转场
- 音频调整：支持片段裁剪（Trim）、倍速（0.1x~16x）、音量（0%~1000%）、音调（25%~400%）和音频淡化
- 导出能力：
